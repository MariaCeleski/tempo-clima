...
export function SearchHistory({ history, onSelect, onClear }: SearchHistoryProps) {
  ...
  return (
    <nav className="mt-3 rounded-lg border border-slate-200 dark:border-white/15 bg-white/70 dark:bg-white/5 p-3 shadow-sm dark:shadow-none backdrop-blur-sm" aria-label="Buscas recentes">
      <div className="mb-2 flex items-center justify-between">
        <span className="text-xs font-medium text-slate-500 dark:text-white/60" id="search-history-label">Buscas recentes</span>
        <button
          onClick={onClear}
